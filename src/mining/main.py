...
def bitcoin_mining_record():
            ...
            async with aiohttp.ClientSession() as session:
                ...
                if request_price:
                    ...
                    price_task = asyncio.create_task(
                        fetch(session, price_url, params["ApiAlarmThreshold"])
                    )

                if request_hashrate:
                    next_request_hashrate_time = (
                        spider_ts + params["FetchHashrateInterval"]
                    )
                    Variable.set(
                        "next_request_hashrate_time",
                        next_request_hashrate_time
                    ...
                    hashrate_task = asyncio.create_task(
                        fetch(session, hashrate_url,
            ...
            trans_data = {
                ...
                "hashrate": (
                    Hashrate(
                        ...
                        difficulty=str(
                            api_data["hashrate"]["currentDifficulty"]),
                        server_ts=api_data["spider_ts"],
                    )
                    if api_data["hashrate"] is not None
                    else None
                ),
            }
            return trans_data
    ...
    def mysql_trans(model_data: dict, mysql_hook: Connection):
        ...
        items = [model_data.get("price", None),
                 model_data.get("hashrate", None)]

        # init db conn
        mysql_cursor: Cursor = mysql_hook.cursor()
        for item in items:
            if item:
                mysql_cursor.execute(item.SQL_INSERT,
                                     (*item.sql_args, spider_ts))
                mysql_hook.commit()
    ...
    def load(params: dict = None):
        ...
        if not next_load_time:
            ...
            if spider_ts >= int(next_load_time):
                ...
                try:
                    mysql_hook: Connection = MySqlHook(
                        mysql_conn_id="mysql_mining"
                    ).get_conn()
                    mysql_cursor: Cursor = mysql_hook.cursor()

                    # Query price data for the last 5 minutes
                    mysql_cursor.execute(
                        Price.SQL_QUERY_AVG, (time_range_start, spider_ts)
                    )
                    data = mysql_cursor.fetchone()
                    # If the current price doesn't exist, try to get last 10
                    # mins data
                    if not data:
                        mysql_cursor.execute(
                            Price.SQL_QUERY_AVG,
                            (time_range_start - load_interval,
                             time_range_start),
                        )
                        data = mysql_cursor.fetchone()
                        if not data:
                            raise RuntimeError(
                                "Mysql no data found in last 10 minutes."
                            )
                    ...
                    mysql_cursor.execute(
                        Hashrate.SQL_QUERY, (time_range_start, spider_ts)
                    )
                    ...
                    mysql_cursor.execute(
                        """
                        INSERT INTO
                        avg_info (USD, hashrate, difficulty, spider_ts)
